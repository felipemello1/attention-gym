...
def test_candidate_bounds_match_document_intervals(causal, ratio):
    ...
    for lengths in ([0], [319], [0, 0], [0, 3, 0, 9, 257, 0, 7], [i % 7 for i in range(33)]):
        ...
        for reference_q, reference_k in ((cu_q, cu_k), (cu_q.cpu(), cu_k.cpu())):
            _, positions, starts, ends = packed_sequence_metadata(reference_q, reference_k, tokens)
            if causal:
...
def test_candidate_bounds_cuda_graph_replays_changed_offsets():
    ...
    graph.replay()
    _, positions, starts, ends = packed_sequence_metadata(cu_q, cu_k, 19)
    ends = torch.minimum(ends, starts + (positions + 1) // 4)
    torch.testing.assert_close(actual, torch.stack((starts, ends), -1))
...
def test_candidate_bounds_forced_int64_offsets(monkeypatch):
    ...
    cu_q = torch.tensor([0, 3, 12, 17], device=device, dtype=torch.int32)
    cu_k = torch.tensor([0, 0, 2, 3], device=device, dtype=torch.int32)
    expected = _varlen_triton.prepare_candidate_bounds(cu_q, cu_k, 19, True, 4)
    monkeypatch.setattr(_varlen_triton, "requires_int64_offsets", lambda *tensors: True)
    actual = _varlen_triton.prepare_candidate_bounds(cu_q, cu_k, 19, True, 4)
