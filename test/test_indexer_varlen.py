...
def test_packed_metadata_validation(case, message):
...
def test_packed_cuda_graph_replays_changed_offsets(backend):
    inputs = _inputs([3, 9, 7], 4, _device(backend), heads=32, dim=128, capacity=3)
    stream = torch.cuda.Stream()
    stream.wait_stream(torch.cuda.current_stream())
    with torch.cuda.stream(stream):
        for _ in range(3):
            _call(backend, inputs, 3, 4)
    torch.cuda.current_stream().wait_stream(stream)
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph, stream=stream):
        actual = _call(backend, inputs, 3, 4)
    q, _, _, cu_q, cu_k = inputs
    for offsets_q, offsets_k in (([0, 7, 12, 19], [0, 1, 2, 3]), ([0, 0, 11, 16], [0, 0, 2, 3])):
