...
@pytest.mark.parametrize(
    "dynamic, strided",
    [
        pytest.param(None, False, id="automatic-dynamic"),
        pytest.param(True, False, id="dynamic-shapes"),
        pytest.param(False, True, id="strided"),
    ],
)
def test_packed_triton_fullgraph_shape_specialization(gather_attn_single_config, dynamic, strided):
    """Specialize changing physical shapes without losing static-graph strided KV support."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA required")

    compiled = torch.compile(gather_attn, fullgraph=True, dynamic=dynamic)
    for capacity in (3, 5):
        ...
        if strided:
            ...
            for i in (1, 2):
                tensors[i] = (
                    tensors[i].detach().repeat_interleave(2, dim=2)[:, :, ::2].requires_grad_()
                )
        q, local, sparse, sink = tensors
        kwargs = {
            "sliding_window_size": 7,
...
def test_reverse_map_preserves_local_selection_multiplicity(packed, dtype):
    ...
    compiled = torch.compile(_build_index_query_map, fullgraph=True, dynamic=True)
    for tokens in (9, 11):
        ...
        queries, bounds = compiled(indices, 6, cu_q, cu_k)
