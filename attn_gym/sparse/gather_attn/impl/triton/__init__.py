...
class _GatherAttnFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if torch.compiler.is_compiling():
            # The kernels specialize on shapes and constexpr stride tuples. Specialize
            # inside this autograd boundary so shape-derived strides are concrete here.
            for tensor in (query, local_kv, sparse_kv, kv_indices):
...
def gather_attn(
    ...
    *,
    ...
    Compilation specializes attention/index tensor shapes and shape-derived strides;
    changing physical shapes can recompile even with ``dynamic=True``. Layouts with
    independently symbolic strides require ``dynamic=False``. Packed offset contents
    remain device-side runtime inputs.
    ...
    if torch.compiler.is_compiling():
        # Inference bypasses the autograd wrapper but needs the same specialization.
        for tensor in (query, local_kv, sparse_kv, kv_indices):
            torch._dynamo.mark_static(tensor)
    if share_kv:
